...
function! LoadTemplate()
	...
	let s:ext = expand('%:e')
    let s:low_ext = tolower(s:ext)
    if s:low_ext == "c" || s:low_ext == "cc" || s:low_ext == "cxx" || s:low_ext == "c++"
        let s:ext = "cpp"
    endif
    if s:low_ext == "hpp"
        let s:ext = "h"
...
function! TemplateReplTags()
	...
	unlet s:fn
    " add by smashell
    let s:header_h = toupper(substitute(expand("%:t"), '\.', '_', 'g'))."_"
    sil! execute sl.','.el."s/<T_HEADER_H>/".s:header_h."/g"
    let s:header_file = substitute(expand("%:t"), '\.\a\+$', '\.h', 'g')
    sil! execute sl.','.el."s/<T_HEADER_FILE>/".s:header_file."/g"
    call cursor(1,1)
